<?php 
    ...
    $category_id = mysqli_real_escape_string($link,$_GET['category']);
    $subcategory_id = mysqli_real_escape_string($link,$_GET['subcategory']);
    $sql1 = "SELECT DISTINCT name FROM category WHERE id='$category_id'";
    $query1 = mysqli_query($link,$sql1);
    $category_name = mysqli_fetch_assoc($query1)['name'];
    $sql2 = "SELECT DISTINCT name FROM subcategory WHERE id='$subcategory_id'";
    $query2 = mysqli_query($link,$sql2);
    $subcategory_name = mysqli_fetch_assoc($query2)['name'];

    if(isset($_POST['search']) || $_SERVER['REQUEST_METHOD'] == 'POST'){

    }else{
        $sql = "SELECT product.id as product_id,category.name as category,subcategory.name as subcategory,product.name as product_name,price,image,product.status as status,stocks FROM product LEFT JOIN category ON product.cat_id = category.id LEFT JOIN subcategory ON product.subcat_id = subcategory.id WHERE product.status=1 AND product.cat_id='$category_id' AND product.subcat_id='$subcategory_id'";
    }
...
<body>
    ...
    <div class="container">
        ...
        <?php if($_SERVER['REQUEST_METHOD'] == 'POST'){ echo '<h1 class="mt-4 mb-4">Search For '; foreach($_POST as $search){ if(!empty($search)){ ?>
            <span><?= '#'.$search.' ' ?></span>
        <?php }} echo '</h1>'; }else{ ?>
            <h2 class="mt-4 mb-4">Product By Category: <span class="text-primary"><?= isset($category_name) ? $category_name : '' ?></span> & Subcategory: <span class="text-secondary"><?= isset($subcategory_name) ? $subcategory_name : '' ?></span></h2>
        <?php } ?>
        <div class="row">
        <?php 
